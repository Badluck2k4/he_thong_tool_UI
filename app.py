...
if uploaded_file is not None:
    ...
    if results:
        ...
        for idx, v in enumerate(results):
            with cols[idx]:
                # Sử dụng màu xanh lá cây cho Delta để nổi bật
                st.metric(label=f"VỤ {idx+1}", value=f"{v['total']} lần", delta=f"{v['days']} ngày", delta_color="normal")
                st.markdown(f"<p style='color:black;'>Từ {v['start']} đến {v['end']}</p>", unsafe_allow_html=True)
        ...
        for idx, v in enumerate(results):
            ...
            # Đảm bảo biểu đồ có màu sắc tương phản tốt
            fig, ax = plt.subplots(figsize=(12, 4))
            ax.bar(dates, counts, color='#2ca02c')
            ax.set_facecolor('#ffffff')
            ax.axhline(y=freq_val, color='red', linestyle='--', alpha=0.5)
